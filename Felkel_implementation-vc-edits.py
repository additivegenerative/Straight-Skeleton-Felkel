...
class _LAVertex:
	...
	def next_event(self):
		...
		i_prev = self.bisector.intersect(self.prev.bisector) #intersection of current vertex bisector with previous vertex bisector
		i_next = self.bisector.intersect(self.next.bisector) #intersection of current vertex bisector with next vertex bisector

		if i_prev is not None:
			events.append(_EdgeEvent(Line2(self.edge_left).distance(i_prev), i_prev, self.prev, self)) #_EdgeEvent named tuple
